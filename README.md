...
**Course materials.** When `materialsDir` is set, the card shows a *Course Materials* button that opens `/teaching/<materialsDir>/`, a page generated at build time from the contents of `public/teaching/<materialsDir>/`. To publish material, drop the files (PDF, ZIP, notebooks, ...) in that folder and rebuild; no other change is needed. Files are listed in name order, so a numeric prefix controls the sequence. The display title is derived from the file name: the numeric prefix is dropped, `--` becomes a colon, single `-`/`_` become spaces and the original capitalisation is kept.
